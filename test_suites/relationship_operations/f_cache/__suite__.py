name = "Cache (E)" 
description = "Edge Ingestion as a function of search size set and cache size."
...
for engine in ["ig2","ig3"]:
    cases.append({
        "name":"ingest",
        "description":"Ingestion for navigation purposes (threads=%d,txsize=%d,page_size=%d)"%(threads,txsize,pow(2,page_size)),
        "type":"graph_v_ingest",
        "data":
        {
    ...
    cases.append({
        "name":"navigate",
        "description":"Navigation as a function of cache size and search size set (graph_size=%d,txsize=%d,page_size=%d)"%(graph_size[0],txsize,pow(2,page_size)),
        "type":"graph_v_search",
        "data":
        {
            "graph_size":graph_size,
